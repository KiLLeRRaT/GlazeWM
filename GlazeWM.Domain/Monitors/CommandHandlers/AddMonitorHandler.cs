...
namespace GlazeWM.Domain.Monitors.CommandHandlers
{
  internal class AddMonitorHandler : ICommandHandler<AddMonitorCommand>
  {
    ...
    private void ActivateWorkspaceOnMonitor(Monitor monitor)
    {
      ...
      var inactiveWorkspaceName = _workspaceService.GetInactiveWorkspaceNameForMonitor(monitor) ?? _workspaceService.GetInactiveWorkspaceNames().ElementAtOrDefault(0);
