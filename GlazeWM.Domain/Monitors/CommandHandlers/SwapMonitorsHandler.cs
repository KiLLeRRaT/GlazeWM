...
namespace GlazeWM.Domain.Monitors.CommandHandlers
{
  public class SwapMonitorsHandler : ICommandHandler<SwapMonitorsCommand>
  {
    ...
    public CommandResponse Handle(SwapMonitorsCommand command)
    {
      ...
      var displayedWorkspaces = MoveWorkpaces(command.Direction);
    ...
    private List<Workspace> MoveWorkpaces(Direction direction)
    {
      var focusedMonitor = _monitorService.GetFocusedMonitor();
      List<Workspace> displayedWorkspaces = new List<Workspace>();
      Monitor monitor = _monitorService.GetMonitorInDirection(direction, focusedMonitor);

      List<Workspace> nextMonitorsChildren = monitor.Children.Cast<Workspace>().ToList();
